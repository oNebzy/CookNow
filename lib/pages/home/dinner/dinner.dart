...
class Dinner extends StatefulWidget {
  ...
  @override
  _DinnerState createState() => _DinnerState(id);
}
...
class _DinnerState extends State<Dinner> {
  ...
  String? id = '';
  _DinnerState(this.id);
  FirebaseFirestore curr = FirebaseFirestore.instance;
  List<Recipe> recipes = [];

  Future<void> getData() async{
    print('$id');
    await curr.collection('users').doc('$id').collection('dinner').get().then((querySnapshot) {
  ...
  Widget build(BuildContext context) {
              ...
              children: [
                ...
                TextButton.icon(
                    ...
                    onPressed: () async {
                      ...
                      DatabaseService(widget.id!).addUserData('dinner', name, ingred, instrucs);
    ...
    return StreamProvider<List<Recipe>?>.value(
      initialData: null,
      value: DatabaseService(widget.id!).dinner,
      child: Scaffold(
        backgroundColor: Colors.blue[700],
        appBar: AppBar(
          backgroundColor: Colors.blue[900],
          title: Text('Dinner Recipe List'),
          centerTitle: false,
          elevation: 1,
          actions: [
            Padding(
